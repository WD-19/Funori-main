...
<head>
    ...
    <style>
        @page { margin: 20mm; }
        body { font-family: 'DejaVu Sans', Arial, sans-serif; margin: 0; padding: 0; font-size: 14px; color: #333; }
        .shipping-box { max-width: 800px; margin: auto; border:1px solid #ccc; padding:30px; }
        .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #eee; }
        .company-logo img { max-height: 50px; margin-bottom: 10px; }
        .company-info, .customer-info { width: 48%; }
        .company-info strong, .customer-info strong { display: block; margin-bottom: 5px; font-size: 16px; }
        .title { font-size: 28px; font-weight: bold; text-align: center; margin: 20px 0; }
        .order-meta { text-align: center; margin-bottom: 25px; color: #555; }
        .items table { width: 100%; border-collapse: collapse; }
        .items th, .items td { border: 1px solid #ddd; padding: 10px; text-align: left; }
        .items th { background: #f9f9f9; font-weight: bold; }
        .items td.center, .items th.center { text-align: center; }
        .notes { margin-top: 30px; padding: 15px; border: 1px dashed #ccc; border-radius: 5px; background: #fdfdfd; }
        .signatures { margin-top: 50px; display: flex; justify-content: space-around; text-align: center; }
        .signatures div { width: 45%; }
        .signatures .signature-line { border-bottom: 1px solid #333; height: 40px; margin-top: 40px; }
...
<div class="shipping-box">
    ...
    <div class="header">
        <div class="company-info">
            ...
            Địa chỉ: Tòa nhà Funori, Quận 1, TP. HCM<br>
            SĐT: 1900 1234
        </div>
                        ...
                        @elseif($item->product_variant_id && $item->productVariant && isset($item->productVariant->attributeValues))
                             <br><small style="color:#555;">
                                @foreach($item->productVariant->attributeValues as $attrValue)
                                    <span>{{ optional($attrValue->attribute)->name ?? '' }}: {{ $attrValue->value ?? '' }}</span>@if(!$loop->last), @endif
                                @endforeach
                            </small>
                        @endif
                    </td>
