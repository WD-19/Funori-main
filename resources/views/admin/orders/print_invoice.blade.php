...
<head>
    ...
    <style>
        ...
        body { font-family: 'DejaVu Sans', Arial, sans-serif; margin: 0; padding: 0; font-size: 14px; }
        ...
        .info, .items, .totals, .notes { margin-bottom: 20px; }
        .info div, .invoice-meta div { margin: 4px 0; }
...
<div class="invoice-box">
    ...
    <div class="invoice-meta">
        ...
        <div><strong>Ngày tạo:</strong> {{ $order->ordered_at ? $order->ordered_at->format('d/m/Y H:i') : '-' }}</div>
        <div><strong>Phương thức thanh toán:</strong> {{ optional($order->paymentMethod)->name ?? ($order->payment_method ?? 'Chưa xác định') }}</div>
  ...
  <td>
    ...
    <div class="notes">
        <strong>Ghi chú:</strong> Cảm ơn Quý khách đã mua hàng tại FUNORI. Vui lòng giữ lại hóa đơn để đối chiếu khi cần đổi trả.
    </div>
