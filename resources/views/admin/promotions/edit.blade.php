    ...
    <form class="form-edit-promotion" method="POST" action="{{ route('admin.promotions.update', $promotion->id) }}">
        ...
        <div class="wg-box mb-30">
            <fieldset class="name">
                ...
                <input class="mb-10" type="text" placeholder="Nhập tên khuyến mãi" name="name" maxlength="100" required value="{{ old('name', $promotion->name) }}" id="name">
                @error('name')
            ...
            <fieldset class="code">
                ...
                <input class="mb-10" type="text" placeholder="Nhập mã" name="code" maxlength="50" required value="{{ old('code', $promotion->code) }}" id="code">
                @error('code')
                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                @enderror
            </fieldset>
            <fieldset class="discount_type">
                ...
                <select name="discount_type" id="discount_type" required>
                    ...
                    <option value="fixed_amount" {{ old('discount_type', $promotion->discount_type) == 'fixed_amount' ? 'selected' : '' }}>Tiền mặt (VNĐ)</option>
                </select>
                @error('discount_type')
                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                @enderror
            ...
            <fieldset class="discount_value">
                ...
                <input type="number" name="discount_value" min="0" step="0.01" required value="{{ old('discount_value', $promotion->discount_value) }}">
            ...
            <fieldset class="max_discount_amount" id="max_discount_amount_field">
                ...
                <input type="number" name="max_discount_amount" min="0" step="0.01" value="{{ old('max_discount_amount', $promotion->max_discount_amount) }}">
            ...
            <fieldset class="min_order_value">
                ...
                <input type="number" name="min_order_value" min="0" step="0.01" required value="{{ old('min_order_value', $promotion->min_order_value) }}">
            ...
            <fieldset class="usage_limit_per_voucher">
                ...
                <input type="number" name="usage_limit_per_voucher" min="0" required value="{{ old('usage_limit_per_voucher', $promotion->usage_limit_per_voucher) }}">
            ...
            <fieldset class="usage_limit_per_user">
                ...
                <input type="number" name="usage_limit_per_user" min="0" required value="{{ old('usage_limit_per_user', $promotion->usage_limit_per_user) }}">
            ...
            <fieldset class="start_date">
                ...
                <input type="date" name="start_date" required value="{{ old('start_date', $promotion->start_date ? date('Y-m-d', strtotime($promotion->start_date)) : '') }}">
                @error('start_date')
                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                @enderror
            </fieldset>
            <fieldset class="end_date">
                ...
                <input type="date" name="end_date" required value="{{ old('end_date', $promotion->end_date ? date('Y-m-d', strtotime($promotion->end_date)) : '') }}">
                @error('end_date')
                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                @enderror
            </fieldset>
            <fieldset class="applies_to">
                ...
                <select name="applies_to" id="applies_to" required>
                    <option value="all_products" {{ old('applies_to', $promotion->applies_to) == 'all_products' ? 'selected' : '' }}>Tất cả sản phẩm</option>
                    <option value="specific_brands" {{ old('applies_to', $promotion->applies_to) == 'specific_brands' ? 'selected' : '' }}>Thương hiệu cụ thể</option>
                    <option value="specific_categories" {{ old('applies_to', $promotion->applies_to) == 'specific_categories' ? 'selected' : '' }}>Danh mục cụ thể</option>
                </select>
                @error('applies_to')
                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                @enderror
