...
{{-- <div class="tf-page-title">...</div> --}}
<div style="max-width: 66vw; margin: 60px auto 0 auto; padding: 0 16px;">
    <div class="cart-checkout-progress" style="background: #fff; padding: 32px 16px 24px 16px; border-radius: 10px; margin-bottom: 16px;">
        <div style="display: flex; align-items: flex-start; justify-content: space-between;">
            <div style="flex:1; text-align: center;">
                <div style="position: relative; display: flex; flex-direction: column; align-items: center;">
                    <div style="width: 48px; height: 48px; background:rgb(255, 48, 41); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; position: relative; z-index: 2;">
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M6 13l4 4 8-8" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </div>
                    <div style="height: 4px; width: 100%; background:rgb(255, 48, 41); position: absolute; top: 24px; left: 48px; right: -50%; z-index: 1;"></div>
                </div>
                <div style="font-weight: bold; color: #232323; margin-top: 8px;">Giỏ hàng</div>
            </div>
            <div style="flex:1; text-align: center; opacity: 0.4;">
                <div style="position: relative; display: flex; flex-direction: column; align-items: center;">
                    <div style="width: 48px; height: 48px; background: #fc9999; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; position: relative; z-index: 2;">
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M6 13l4 4 8-8" stroke="#BDBDBD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </div>
                    <div style="height: 4px; width: 100%; background:#fc9999; position: absolute; top: 24px; left: 48px; right: -50%; z-index: 1;"></div>
                </div>
                <div style="font-weight: bold; color: #676363; margin-top: 8px;">Thanh toán</div>
            </div>
            <div style="flex:1; text-align: center; opacity: 0.4;">
                <div style="position: relative; display: flex; flex-direction: column; align-items: center;">
                    <div style="width: 48px; height: 48px; background:#fc9999; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; position: relative; z-index: 2;">
                ...
                <div style="font-weight: bold; color: #676363; margin-top: 8px;">Hóa đơn</div>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="tf-page-cart-wrap">
                    <div class="tf-page-cart-item">
                        <form>
                            <table class="tf-table-page-cart cart-table-fixed">
                                ...
                                @forelse($cartItems as $item)
                                    <tr class="tf-cart-item file-delete">
                                        <td class="tf-cart-item_checkbox" style="width: 36px; padding-right: 4px; vertical-align: middle;">
                                        ...
                                        <td class="cart-img-col" style="width: 90px; vertical-align: middle;">
                                            <div style="position: relative; width: 80px; height: 80px;">
                                                <a href="{{ route('client.product.show', $item['product']['slug'] ?? $item['product_id']) }}" class="img-box">
                                                    <img src="{{ asset($item['product']['images'][0]['image_url'] ?? 'images/products/no-image.png') }}" alt="img-product" style="width: 80px; height: 80px; object-fit: cover; border: 1px solid #eee; border-radius: 6px;">
                                                </a>
                                                <span class="remove-cart-x" data-item-id="{{ $item['id'] }}" style="position: absolute; top: -16px; left: -16px; background: #fff; color: #ff3029; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: bold; box-shadow: 0 1px 4px rgba(0,0,0,0.08); cursor: pointer; border: 1px solid #eee;">&times;</span>
                                            </div>
                                        </td>
                                        <td class="cart-info-col" style="vertical-align: middle;">
                                            <div class="cart-info">
                                                <div class="cart-title-wrap">
                                                    <a href="{{ route('client.product.show', $item['product']['slug'] ?? $item['product_id']) }}" class="cart-title link">
                                                        {{ $item['product']['name'] ?? 'Sản phẩm đã xóa' }}
                                                    </a>
                                                </div>

                                                @if(!empty($item['product']['variants']))
                                                    {{-- Debug: {{ json_encode($item['product']['variants']) }} --}}
                                                    <div class="cart-meta-variant" style="margin-top: 4px;">
                                                        <label style="font-size: 15px; color: #666; font-weight: 400; display: flex; align-items: center; cursor: pointer;">
                                                            Phân Loại Hàng:
                                                            <select class="cart-variant-select" data-item-id="{{ $item['id'] }}" style="border: none; background: transparent; font-size: 15px; color: #666; font-weight: 400; margin-left: 4px; cursor: pointer; appearance: none; outline: none; padding-right: 18px;">
                                                                @foreach($item['product']['variants'] as $variant)
                                                                    <option value="{{ $variant['id'] }}" @if($variant['id'] == ($item['product_variant_id'] ?? $item['product']['variants'][0]['id'])) selected @endif>
                                                                        {{ $variant['size'] }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                            <span class="similar-search-icon" style="margin-left: -18px; pointer-events: none; font-size: 18px; color: #888;">▼</span>
                                                        </label>
                                                        <div style="font-size: 16px; color: #888; margin-top: 2px;">
                                                            @if(isset($item['product_variant_id']))
                                                                {{
                                                                    collect($item['product']['variants'])->firstWhere('id', $item['product_variant_id'])['size'] ?? ''
                                                                }}
                                                            @else
                                        ...
                                        <td class="cart-qty-col" style="text-align: center; vertical-align: middle;">
                                            <div class="cart-quantity">
                                                <div class="wg-quantity">
                                                    ...
                                                    <input type="text" class="cart-qty-input" data-item-id="{{ $item['id'] }}" value="{{ $item['quantity'] }}" min="1">
                            ...
                            <div class="cart-action-buttons" style="display: flex; justify-content: space-between; align-items: center; margin-top: 18px; gap: 10px;">
                                <div class="left-actions">
                                    <a href="{{ route('shop') }}" class="tf-btn" style="background: #232323; color: #fff; font-weight: bold; border: none; display: flex; align-items: center; padding: 12px 28px;">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20" style="margin-right: 8px;"><path d="M12.5 15l-5-5 5-5" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                      Tiếp tục mua sắm
                                    </a>
                                </div>
                                <div class="right-actions" style="display: flex; gap: 10px;">
                                    <button type="button" class="tf-btn delete-selected-btn" id="delete-selected-btn" style="background: #ff3029; color: #fff; border: none; font-weight: bold; padding: 12px 28px; display: none;">Xóa sản phẩm đã chọn</button>
                                    <button type="button" class="tf-btn" id="clear-cart-btn" style="background: #fff; color: #232323; border: 1px solid #232323; font-weight: bold; padding: 12px 28px;">Xóa giỏ hàng</button>
                                </div>
                            </div>
                            ...
                            <div class="tf-page-cart-checkout">
                                ...
                                <div style="font-family: 'Inter', Arial, Helvetica, sans-serif;">
                                    <div class="tf-cart-totals-discounts">
                                        ...
                                        <span class="total-value" style="font-size: 18px;">{{ number_format($total, 0, ',', '.') }}đ</span>
                                    </div>

                                    <div class="tf-cart-discount-fee" style="margin-top: 24px;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                                            <span>Giảm giá</span>
                                            <span class="discount-value">-{{ isset($discount) ? number_format($discount, 0, ',', '.') : '0' }}đ</span>
                                        </div>
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                                            <span>Phí vận chuyển</span>
                                            <span class="shipping-fee-value">{{ isset($shipping_fee) ? number_format($shipping_fee, 0, ',', '.') : '0' }}đ</span>
                                        </div>
                                        <hr style="margin: 8px 0;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; font-weight: bold; font-size: 18px; color: #ff3029;">
                                            <span>Tổng cộng</span>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateSelectedTotal() {
        ...
        if (totalValueElement) {
            if (selectedItems.length === 0) {
                ...
                const grandTotalElement = document.querySelector('.grand-total-value');
                if (grandTotalElement) {
                    grandTotalElement.textContent = (selectedTotal - discount + shippingFee).toLocaleString('vi-VN') + 'đ';
                }
            }
        }
        ...
        selectedItems.forEach(function(checkbox) {
            ...
            .then(data => {
                if (data && data.success) {
                    ...
                    console.warn('Server remove failed for item:', itemId, data.message);
                    return { success: false, itemId: itemId, message: data.message };
                }
            }).catch(error => {
                console.warn('Server remove failed for item:', itemId, error);
                return { success: false, itemId: itemId, error: error };
            });
    ...
    // Thay đổi biến thể trong giỏ hàng
    document.querySelectorAll('.cart-variant-select').forEach(function(select) {
        select.addEventListener('change', function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || 'Có lỗi xảy ra khi cập nhật biến thể!');
                }
            })
            .catch(error => {
