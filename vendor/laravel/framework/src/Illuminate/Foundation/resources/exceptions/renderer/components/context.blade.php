        ...
        @forelse ($exception->applicationRouteContext() as $name => $value)
            <div class="flex items-center gap-2 {{ $loop->first ? '' : 'border-t' }} dark:border-gray-800">
                <span data-tippy-content="{{ $name }}"
                    class="lg:text-md w-[8rem] flex-none cursor-pointer truncate border-r px-5 py-3 text-sm dark:border-gray-800 lg:w-[12rem]">{{ $name }}</span>
                <span class="min-w-0 flex-grow"
                    style="
                        -webkit-mask-image: linear-gradient(90deg, transparent 0, #000 1rem, #000 calc(100% - 3rem), transparent calc(100% - 1rem));
                    ">
    ...
    <dl class="mt-1 grid grid-cols-1 rounded border dark:border-gray-800">
        @forelse ($exception->applicationQueries() as $query)
            <div class="flex items-center gap-2 {{ $loop->first ? '' : 'border-t' }} dark:border-gray-800">
                ...
                -webkit-mask-image: linear-gradient(90deg, transparent 0, #000 1rem, #000 calc(100% - 3rem), transparent calc(100% - 1rem));
            ">
                    <pre class="scrollbar-hidden overflow-y-hidden text-xs lg:text-sm">
                <code class="px-5 py-3 overflow-y-hidden scrollbar-hidden max-h-32 overflow-x-scroll scrollbar-hidden-x">
                    {{ $query['sql'] }}
                </code>
            </pre>
                </span>
        ...
        @empty
            ...
            <code class="overflow-y-hidden scrollbar-hidden overflow-x-scroll scrollbar-hidden-x">No query data</code>
        </pre>
            </span>
        @endforelse
